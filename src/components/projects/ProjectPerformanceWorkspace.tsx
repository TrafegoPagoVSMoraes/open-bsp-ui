...
export default function ProjectPerformanceWorkspace() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem(`openbsp-active-project-${orgId}`);
    setProjectId(projects.some((item) => item.id === saved) ? saved! : projects[0].id);
  ...
  }, [orgId, projectId, setActiveProject]);
    ...
    <div className="h-full overflow-y-auto bg-background p-5 text-foreground">
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <header className="flex flex-wrap items-center justify-between gap-3">
          ...
          <select className="text min-w-64" value={projectId} onChange={(event) => { setProjectId(event.target.value); setPreview(null); setPreviewExistingDates([]); }}>
        ...
        <section className="rounded-xl border border-border bg-card p-4">
          ...
          {project && <div className="mt-4 grid gap-3 md:grid-cols-2">
